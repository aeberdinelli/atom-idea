    ...
    prepare() {
        this.project.forEach(path => {
            walk_promise(path).then(files => {
                ...
                files.forEach(file => this.promises.push(analyze(file)));

                return Promise.all(this.promises).then(results => {
                    results.forEach(result => {
                        this.functions = this.functions.concat(result.functions);
                        this.variables = this.variables.concat(result.variables);
                        this._classes = this._classes.concat(result._classes);
                    });

                    atom.notifications.addSuccess('Found ' + this.functions.length + ' functions, ' + this._classes.length + ' classes');
                });
            })
            .catch(err => {
                console.error(err);
